...
export const SettingsProviders: Component = () => {
  ...
  const fallbackModel = (provider: ReturnType<typeof normalizeProviderList>) => {
    ...
    const first = [...provider.all.values()].find(
  ...
  const refreshProviders = async () => {
    ...
    const directories = new Set(
      serverSync.data.project.flatMap((project) => [project.worktree, ...(project.sandboxes ?? [])]).filter(Boolean),
    )
    if (dir()) directories.add(dir())
    for (const directory of directories) {
      const [, setProjectStore] = serverSync.child(directory, { bootstrap: false })
  ...
  const disconnect = async (providerID: string, name: string) => {
    ...
    await serverSDK.client.auth
      ...
      .then(async () => {
        ...
        const normalized = await refreshProviders()
        if (
          normalized &&
          providerID === "aifactory" &&
          serverSync.data.config.model?.startsWith("aifactory/")
        ) {
