    ...
    .then(async (res) => {
      ...
      return Object.fromEntries(
        Object.entries(payload.mcp ?? {}).flatMap(([name, config]) => {
          ...
          const parsed = Schema.decodeUnknownExit(ConfigMCP.Info)(rest)
          if (!parsed._tag || parsed._tag !== "Success") return []
          return [
            [name, { config: parsed.value, auth: parseManagedAuth((config as Record<string, unknown>).auth) }] as const,
          ]
        }),
      )
    })
