...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      let data = ConfigParse.schema(Info, normalizeLoadedConfig(parsed, source), source)
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const remoteModel = typeof remoteProviderConfig.model === "string" ? remoteProviderConfig.model : undefined
        const remoteSmallModel =
          typeof remoteProviderConfig.small_model === "string" ? remoteProviderConfig.small_model : undefined
        if (remoteModel !== undefined || remoteSmallModel !== undefined) {
          yield* merge(
            "updater-provider-config",
            {
              ...(remoteModel !== undefined ? { model: remoteModel } : {}),
              ...(remoteSmallModel !== undefined ? { small_model: remoteSmallModel } : {}),
            },
            "global",
          )
        }
    ...
    const updateGlobal = Effect.fn("Config.updateGlobal")(function* (config: Info) {
      ...
      const next = ConfigParse.schema(Info, ConfigParse.jsonc(finalized, file), file)
